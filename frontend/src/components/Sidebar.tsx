    ...
    <aside className="w-60 flex-shrink-0 flex flex-col bg-primary border-r border-border h-screen overflow-y-auto">
      ...
      <div className="p-4 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="info">v{version || '1.0.0'}</Badge>
        </div>
      ...
      {episodeState.isRunning && (
        <div className="px-3 py-3 border-t border-border space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <Stat icon={<TrendingUp className="w-3 h-3" />} label="Reward" value={episodeState.totalReward.toFixed(2)} positive={episodeState.totalReward >= 0} />
            <Stat icon={<Activity className="w-3 h-3" />} label="Steps" value={String(episodeState.steps)} />
          </div>
        </div>
      )}
      ...
      <div className="p-3 space-y-2 border-t border-border">
        <Button onClick={onReset} isLoading={isLoading} className="w-full" size="sm">
          <RotateCcw className="w-3.5 h-3.5" /> Reset Episode
        </Button>
        {episodeState.isDone && (
          <Button variant="outline" onClick={onGrade} isLoading={isLoading} className="w-full" size="sm">
